...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Получаем master_id (может быть NULL если не выбран)
        $masterId = !empty($_POST['master_id']) ? (int)$_POST['master_id'] : null;

        // Создаем заказ с правильными полями (ИСПРАВЛЕННЫЙ ЗАПРОС)
        $sql = "INSERT INTO orders (
            client_id, device_category_id, brand_id, device_model_id, custom_device_model,
            problem_description, manager_id, master_id, status_id, source_id, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        $params = [
            ...
            $masterId,             // ВАЖНО: master_id (может быть NULL)
            (int)$_POST['status_id'],
            (int)$_POST['source_id']
        ];
        ...
        if ($result) {
            ...
            // Добавляем комментарий о назначении мастера
            if ($masterId) {
                // Получаем имя мастера для комментария
                $masterStmt = $pdo->prepare("SELECT full_name FROM users WHERE id = ?");
                $masterStmt->execute([$masterId]);
                $master = $masterStmt->fetch(PDO::FETCH_ASSOC);
                $masterName = $master ? $master['full_name'] : 'неизвестен';
                
                $commentStmt = $pdo->prepare("INSERT INTO order_comments (order_id, user_id, comment) VALUES (?, ?, ?)");
                $commentStmt->execute([$orderId, $_SESSION['user_id'], "Назначен мастер: {$masterName}"]);
            }
