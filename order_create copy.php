...
// НИКАКИХ echo или выводов ДО этого момента!
...
echo "Шаг 5: Проверяем авторизацию...<br>";
flush();

requireAuth();
echo "Шаг 6: Авторизация пройдена<br>";
flush();

// Проверяем права доступа
echo "Шаг 7: Проверяем права доступа...<br>";
flush();

requirePermission('orders:create');
echo "Шаг 8: Права доступа есть<br>";
flush();

// Проверяем подключение к базе
echo "Шаг 9: Проверяем базу данных...<br>";
flush();
global $pdo;
...
echo "Шаг 10: База данных подключена<br>";
flush();
// Проверяем SQL запросы
echo "Шаг 11: Проверяем SQL запросы...<br>";
flush();

try {
    // Простой тестовый запрос
    $test = $pdo->query("SELECT 1 as test")->fetch();
    echo "Шаг 12: Тестовый запрос выполнен<br>";
    flush();
    
    // Проверяем существование таблиц
    $tables = ['device_categories', 'brands', 'users', 'statuses', 'client_sources', 'age_groups'];
    
    foreach ($tables as $table) {
        try {
            $result = $pdo->query("SELECT COUNT(*) FROM $table LIMIT 1");
            echo "Шаг 13: Таблица $table доступна<br>";
            flush();
        } catch (Exception $e) {
            echo "Шаг 13: ОШИБКА - Таблица $table не существует: " . $e->getMessage() . "<br>";
            flush();
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Включаем вывод всех ошибок на экран
    echo '<div style="background: #f8d7da; padding: 10px; margin: 10px; border: 1px solid #f5c6cb; display: none;" id="debugInfo">';
    ...
    try {
        ...
        echo '<p>Обязательные поля заполнены</p>';

        // ПРОСТАЯ ФУНКЦИЯ ДЛЯ ТЕСТИРОВАНИЯ
        function handleClientData($postData) {
            global $pdo;
            
            echo '<p>Функция handleClientData вызвана</p>';
            echo '<p>Телефон: ' . htmlspecialchars($postData['client_phone']) . '</p>';
            echo '<p>ФИО: ' . htmlspecialchars($postData['client_full_name']) . '</p>';
            
            $phone = trim($postData['client_phone']);
            $fullName = trim($postData['client_full_name']);
            
            if (empty($phone) || empty($fullName)) {
                echo '<p>Ошибка: пустой телефон или ФИО</p>';
                return false;
            }
            
            try {
                echo '<p>Пробуем создать клиента...</p>';
                
                // Простой INSERT без сложных проверок
                $sql = "INSERT INTO clients (full_name, phone, created_at) VALUES (?, ?, NOW())";
                echo '<p>SQL: ' . $sql . '</p>';
                
                $stmt = $pdo->prepare($sql);
                $result = $stmt->execute([$fullName, $phone]);
                
                if ($result) {
                    $newId = $pdo->lastInsertId();
                    echo '<p>Клиент создан успешно! ID: ' . $newId . '</p>';
                    return $newId;
                } else {
                    $errorInfo = $stmt->errorInfo();
                    echo '<p>Ошибка SQL: ' . $errorInfo[2] . '</p>';
                    return false;
                }
                
            } catch (PDOException $e) {
                echo '<p>Исключение PDO: ' . $e->getMessage() . '</p>';
                return false;
            }
        }

        echo '<p>Вызываем handleClientData...</p>';
        $clientId = handleClientData($_POST);
        echo '<p>Результат handleClientData: ' . ($clientId ? $clientId : 'false') . '</p>';
        
        if (!$clientId) {
            throw new Exception("Ошибка обработки данных клиента (см. отладочную информацию выше)");
        }

        // УПРОЩЕННЫЙ SQL ДЛЯ ТЕСТИРОВАНИЯ
        echo '<p>Создаем заказ...</p>';
        $sql = "INSERT INTO orders (
            client_id, device_category_id, brand_id, problem_description, 
            manager_id, status_id, source_id, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
    ...
    echo '</div>';
    echo '<script>document.getElementById("debugInfo").style.display = "block";</script>';
}
