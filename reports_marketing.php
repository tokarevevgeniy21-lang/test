...
function getMarketingSources($startDate, $endDate) {
    global $pdo;
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN o.warranty = 1 THEN o.id END) as warranty_orders,
            COALESCE(SUM(CASE WHEN o.warranty = 1 THEN o.total_amount ELSE 0 END), 0) as warranty_revenue
        FROM clients c
...
function getClientRetention($startDate, $endDate) {
    global $pdo;
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            COALESCE(AVG(DATEDIFF(o.created_at, c.created_at)), 0) as avg_first_order_days
        FROM clients c
        LEFT JOIN client_sources cs ON c.source_id = cs.id
        LEFT JOIN orders o ON c.id = o.client_id AND o.created_at BETWEEN ? AND ?
        WHERE c.created_at BETWEEN ? AND ?
    ...
    // Отдельно посчитаем повторных клиентов для каждого источника
    foreach ($sources as &$source) {
        $stmtRepeat = $pdo->prepare("
            SELECT COUNT(*) as repeat_clients
            FROM (
                SELECT c.id
                FROM clients c
                JOIN client_sources cs ON c.source_id = cs.id
                JOIN orders o ON c.id = o.client_id
                WHERE cs.name = ? AND o.created_at BETWEEN ? AND ?
                GROUP BY c.id
                HAVING COUNT(o.id) > 1
            ) as repeaters
        ");
        
        $stmtRepeat->execute([$source['source_name'], $startDate, $endDate]);
        $repeat = $stmtRepeat->fetch();
        $source['repeat_clients'] = $repeat['repeat_clients'] ?? 0;
    }
    
    return $sources;
}

$sources = getMarketingSources($startDate, $endDate);
$demographics = getClientDemographics($startDate, $endDate);
$retention = getClientRetention($startDate, $endDate);

renderHeader('Маркетинговая аналитика');
?>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Маркетинговая аналитика</h5>
            </div>
            <div class="card-body">
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body py-2">
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h6>Эффективность источников привлечения</h6>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $row['warranty_orders'] ?> (<?= number_format($row['warranty_revenue'], 2) ?> ₽)</td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                ...
                <div class="card">
                    <div class="card-header">
                        <h6>Аналитика удержания клиентов</h6>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?= $row['total_clients'] > 0 ? 
                                                number_format(($row['active_clients'] / $row['total_clients']) * 100, 1) . '%' : 
                                                '0%' ?>
                                        </td>
                                        <td><?= round($row['avg_first_order_days'], 1) ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
