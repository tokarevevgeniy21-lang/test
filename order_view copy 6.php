...
saveToTable('order_comments', $commentData);
        ...
        // Специальная обработка для статуса "Выдан" - возврат запчастей
        if ($newStatusId == 5) {
            processIssuedStatus($orderId, $userId);
        }
    // Добавляем комментарий об изменении статуса
        ...
        addOrderComment($orderId, $userId, "Изменен статус: $oldStatusName → $newStatusName", 'status');
